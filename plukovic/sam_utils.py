...
def sample_background(mask):
    background_pixels = torch.nonzero(mask == 0)
    y, x = background_pixels[random.randint(0, background_pixels.size(0) - 1)]
    return y, x


def extract_sam_masks_v1(scene_data, cameras, pixels):
...
def extract_sam_masks_v2(scene_data, cameras, pixels):
